...
public abstract class ObbyListener<T extends ObbyListenerModule<?>> extends ModuleListener<T, MotionUpdateEvent> {
    ...
    public void addCallback(BlockPos pos) {
        Managers.BLOCKS.addCallback(pos, s -> mc.addScheduledTask(() -> placed.remove(pos)));
    ...
    protected boolean updatePlaced() {
        placed.entrySet().removeIf(entry -> System.currentTimeMillis() - entry.getValue() >= module.confirm.getValue());
        return false;
    }
