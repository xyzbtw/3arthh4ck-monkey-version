...
public class ListenerRender extends ModuleListener<KillAura, Render3DEvent> {
    ...
    public void invoke(Render3DEvent event) {
        if (module.targetRender.getValue()) {
    ...
    private void jelloRender(Entity target) {
        ...
        for (int segments = 0; segments < 360; segments += 5) {
            ...
            GL11.glColor4f(pulseColor(color, 200, 1).getRed() / 255.0f, pulseColor(color, 200, 1).getGreen() / 255.0f, pulseColor(color, 200, 1).getBlue() / 255.0f, module.jelloColor.getValue().getAlpha());
