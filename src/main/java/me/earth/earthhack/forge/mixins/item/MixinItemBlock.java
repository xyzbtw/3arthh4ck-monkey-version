...
public abstract class MixinItemBlock {
    ...
    @Dynamic
    @Shadow(remap = false)
    public abstract boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world,
    ...
    @Redirect(
            ...
            at = @At(
                    ...
                    remap = false))
    private boolean onItemUseHook(ItemBlock block, ItemStack stack, EntityPlayer player,
                                  World world, BlockPos pos, EnumFacing facing,
                                  float hitX, float hitY, float hitZ, IBlockState state) {
        return world.isRemote && NO_GLITCH_BLOCKS.isPresent() && NO_GLITCH_BLOCKS.get().noPlace()
                || this.placeBlockAt(stack, player, world, pos, facing, hitX, hitY, hitZ, state);
    }
}