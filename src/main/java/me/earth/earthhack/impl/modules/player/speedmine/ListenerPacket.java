...
public class ListenerPacket extends ModuleListener<Speedmine, PacketEvent.Send> {
    ...
    public void invoke(PacketEvent.Send event) {
        ...
        if(event.getPacket() instanceof CPacketHeldItemChange && module.damages[mc.player.inventory.currentItem] < module.limit.getValue()){
            int id = ((CPacketHeldItemChange) event.getPacket()).getSlotId();
            if(id == InventoryUtil.findHotbarItem(Items.DIAMOND_PICKAXE )) return;
            module.resetCD();
            return;
        }
